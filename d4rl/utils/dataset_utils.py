...
class DatasetWriter(object):
    def __init__(self, mujoco=False, goal=False, hidden_state=False):
        self.mujoco = mujoco
        self.goal = goal
    ...
    def _reset_data(self):
        ...
        if self.hidden_state:
            data['infos/hidden_state'] = []
        return data

    def __len__(self):
        return self._num_samples

    def append_data(self, s, a, r, done, goal=None, mujoco_env_data=None, hidden_state=None):
        self._num_samples += 1
        ...
        if self.hidden_state:
            self.data['infos/hidden_state'].append(hidden_state)
        if self.mujoco:
